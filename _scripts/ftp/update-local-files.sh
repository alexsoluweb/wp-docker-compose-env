...
# SYNOPSYS
#=================================================
# This script is used to mirror remote to local using lftp.
#
...
for index in "${!LOCAL_DIRS[@]}"; do
  ...
  if [ "$1" == "dry-run" ]; then
    ...
    lftp -u "$REMOTE_USER,$REMOTE_PASS" -p $REMOTE_PORT $REMOTE_HOST -e "set ssl:verify-certificate $verify_cert sftp:auto-confirm yes; mirror --only-newer --dry-run $REMOTE_DIR $LOCAL_DIR; quit" > $DRY_RUN_LOG_PATH
  else
    echo "Dry run mode is disabled"
    lftp -u "$REMOTE_USER,$REMOTE_PASS" -p $REMOTE_PORT $REMOTE_HOST -e "set ssl:verify-certificate $verify_cert sftp:auto-confirm yes; mirror --only-newer $REMOTE_DIR $LOCAL_DIR; quit"
  fi
done